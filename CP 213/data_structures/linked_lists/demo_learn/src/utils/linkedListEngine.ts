...
export function executeStatement(
  ...
  try {
    ...
    if (assignHeadMatch) {
      ...
      explanation = `Pointer ${varName} now points to the head of the list.`;
      return { newState, explanation };
    }

    // Pattern: p = NULL or p = null
    const assignNullMatch = trimmed.match(/^(\w+)\s*=\s*(NULL|null)$/i);
    if (assignNullMatch) {
      const varName = assignNullMatch[1];
      newState.pointers.set(varName, null);
      explanation = `Pointer ${varName} is now set to NULL.`;
      return { newState, explanation };
    }
    ...
    if (assignPointerMatch) {
      ...
      if (!sourcePtr && sourceVar !== 'head') {
        error = `Pointer ${sourceVar} is not defined or is NULL.`;
      ...
      if (deref) {
        explanation = `Pointer ${targetVar} now points to the node after ${sourceVar}.`;
      } else {
        explanation = `Pointer ${targetVar} now points to the same node as ${sourceVar}.`;
      }
      return { newState, explanation };
    }

    // Pattern: newNode = createNode(value) or newNode = new Node(value)
    const createNodeMatch = trimmed.match(/^(\w+)\s*=\s*(createNode|new\s+Node)\((\d+)\)$/);
    if (createNodeMatch) {
      const [_, varName, _, value] = createNodeMatch;
      const newNode = createNode(parseInt(value));
      allNodes.set(newNode.id, newNode);
      newState.pointers.set(varName, newNode);
      explanation = `Created a new node with value ${value} and assigned it to ${varName}.`;
      return { newState, explanation };
    }
    ...
    if (setLinkMatch) {
      ...
      if (!sourcePtr) {
        error = `Cannot set link: ${sourceVar} is NULL.`;
        return { newState, explanation, error };
      }
      ...
      if (actualSourceNode) {
        ...
        explanation = `Set ${sourceVar}->link to point to ${target === 'NULL' || target === 'null' ? 'NULL' : target}.`;
      } else {
    ...
    if (setHeadMatch) {
      ...
      explanation = `Head pointer now points to the node referenced by ${varName}.`;
      return { newState, explanation };
    }
    ...
    if (deleteMatch) {
      ...
      if (!ptr) {
        error = `Cannot delete: ${varName} is NULL.`;
        return { newState, explanation, error };
      }
      ...
      explanation = `Deleted node ${ptr.id} (value: ${ptr.info}). Memory freed.`;
      return { newState, explanation };
    }

    error = `Unknown statement format: "${trimmed}". Please use valid C-style pointer syntax.`;
    return { newState, explanation, error };
